...
#
# This file is derived from Brax 0.12.3 (envs/wrappers/training.py) and has been modified
# by the CRAX Authors: `wrap` defaults `episode_length` to `None`, and `EpisodeWrapper` accumulates only
# metric keys already present in `episode_metrics` instead of every key except `reward`.
...
class EpisodeWrapper(Wrapper):
  ...
  def step(self, state: State, action: jax.Array) -> State:
    ...
    for metric_name in state.metrics.keys():
        if metric_name in state.info['episode_metrics']:
