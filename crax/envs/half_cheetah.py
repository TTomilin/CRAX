...
#
# This file is derived from Brax 0.12.3 (envs/half_cheetah.py) and has been modified
# by the CRAX Authors: loads the CRAX `envs/assets/safe/` model XML and accepts an `episode_length` argument.
...
class Halfcheetah(PipelineEnv):
  ...
  def __init__(
    ...
    path = epath.resource_path('crax') / 'envs/assets/safe/half_cheetah.xml'
    sys = mjcf.load(path)
