...
#
# This file is derived from Brax 0.12.3 (training/agents/sac/train.py) and has been modified
# by the CRAX Authors: adds pixel-observation handling: `_remove_pixels` for the critic inputs and random
# translation augmentation for vision training.
...
from training import replay_buffers
from training import types
from training.acme import running_statistics
from training.acme import specs
from training.agents.sac import checkpoint
from training.agents.sac import losses as sac_losses
from training.agents.sac import networks as sac_networks
...
def _remove_pixels(obs):
  ...
  state observations. Mirrors training.agents.ppo.train / sac_lag.train.
  """
...
def _random_translate_pixels(obs: Dict[str, jax.Array], key: PRNGKey) -> Dict[str, jax.Array]:
  ...
  See training.agents.sac_lag.train's identical helper for why this
...
def train(
  ...
  metrics_aggregator = metric_logger.MetricsLogger(
      buffer_size=10,
      steps_between_logging=int(training_metrics_steps) if training_metrics_steps else None,
